...
__device__ void reduce_warp(volatile dtype *data, int N, int tid) {
	if (N >= 65) {
...
__global__ void kernel4(dtype *g_idata, dtype *g_odata, unsigned int n) {
	__shared__  dtype scratch[MAX_THREADS];
  	...
  	if (threadIdx.x < 32){
		reduce_warp(scratch, n, threadIdx.x);
  	}
